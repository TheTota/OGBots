...
function AbilityUsageThink()
	...
	if not hasBeenCheeky then
		npcBot:ActionImmediate_Chat("Easy game boys!", true)
		hasBeenCheeky = true
	end
...
function CanCastLagunaBladeOnTarget(npcTarget)
	return npcTarget:CanBeSeen() and npcTarget:IsHero() and (GetBot():HasScepter() or not npcTarget:IsMagicImmune()) and
		not npcTarget:IsInvulnerable()
end
...
function ConsiderLagunaBlade()
	...
	local npcTarget = npcBot:GetTarget()
	if (npcTarget ~= nil and CanCastLagunaBladeOnTarget(npcTarget)) then
		if
			(npcTarget:GetActualDamage(nDamage, eDamageType) > npcTarget:GetHealth() and
				UnitToUnitDistance(npcTarget, npcBot) < (nCastRange + 200))
		 then
