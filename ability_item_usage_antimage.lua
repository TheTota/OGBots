...
-- Fonction principale pour l'utilisation des spells
...
function ConsiderBlink()
    ...
    for _, npcEnemy in pairs(tableNearbyEnemyHeroes) do
        if
            (npcEnemy:IsChanneling() and npcEnemy:GetHealth() < npcEnemy:GetMaxHealth() / 2 and CanCastOffensiveBlink() and
                not npcBot:IsSilenced())
         then
    ...
    if (npcBot:GetActiveMode() == BOT_MODE_ATTACK) then
        ...
        for _, npcEnemy in pairs(tableNearbyEnemyHeroes) do
            if
                (CanCastOffensiveBlink(npcEnemy) and not npcBot:IsSilenced() and
                    GetUnitToUnitDistance(npcBot, npcEnemy) >= 300)
             then
...
function ConsiderManaVoid()
    ...
    if
        (npcTarget ~= nil and CanCastManaVoidOnTarget(npcTarget) and
            npcTarget:GetHealth() <
    ...
    if (#tableNearbyAttackingAlliedHeroes >= 2) then
        ...
        if (mostDamageableEnemy ~= nil) then
